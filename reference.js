...
const updateTimeExtentCache = (url, token) =>
  new Promise((resolve, reject) => {
    request(
      {
  ...
  app.get('/', async (req, res) => {
    ...
    try {
      ...
      sourcedata.results.forEach((element, index) => {
        ...
        const existingUpdate = arrUpdates.find(
          (x) => x.issueFor === issueForDate
        );

        if (!existingUpdate) {
          element.declareList.forEach((declare) => {
            const currentFeature = featureData.features.find(
              (x) =>
                x.attributes.TFB_DIST.toLowerCase() ===
                  declare.name.toLowerCase() &&
                x.attributes.ForecastID === index + 1
            );
          ...
          element.declareList.forEach((declare) => {
            const currentFeature = featureData.features.find(
              (x) =>
                x.attributes.TFB_DIST.toLowerCase() ===
                  declare.name.toLowerCase() &&
                x.attributes.ForecastID === existingUpdate.forecastId
            );

            const currentUpdateIndex = arrUpdates.findIndex(
              (x) => x.objectId === currentFeature.attributes.OBJECTID
            );
